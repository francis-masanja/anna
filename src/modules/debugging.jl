...
function find_performance_bottlenecks(code::String)::Vector{String}
    ...
    if occursin("println", code) && occursin("for ", code)
        push!(suggestions, "Printing in loops can significantly slow down performance. Consider accumulating output.")
    end
    
    if occursin("global", code)
        push!(suggestions, "Avoid global variables - use function arguments and return values instead")
    end
...
function get_error_types()::Vector{String}
    return ["UndefinedVar", "MethodError", "DomainError", "ArgumentError", 
            "BoundsError", "TypeError", "SyntaxError", "LoadError", "RuntimeError", "Other"]
end
